...
def build_group_plans(socket_map: dict[int, dict[int, list[int]]], groups_per_socket: int) -> list[GroupPlan]:
    plans: list[GroupPlan] = []
    for socket in sorted(socket_map):
        nodes = sorted(socket_map[socket])
        if len(nodes) < 2:
            raise RuntimeError(f"socket {socket} has fewer than 2 NUMA nodes")
        primary_node, secondary_node = nodes[0], nodes[1]
        primary_cpus = socket_map[socket][primary_node]
        secondary_cpus = socket_map[socket][secondary_node]
        groups = chunk_list(primary_cpus, groups_per_socket)
        for idx, group_cpus in enumerate(groups):
            plans.append(
                GroupPlan(
                    name=f"socket{socket}_group{idx}",
                    socket=socket,
                    group_index=idx,
                    primary_node=primary_node,
                    secondary_node=secondary_node,
                    primary_cpus=group_cpus,
                    secondary_cpus=secondary_cpus,
                    # Memory interleave across both NUMA nodes in the socket.
                    mem_nodes=[primary_node, secondary_node],
                )
            )
    return plans
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NUMA binding helper for Ascend A2/A3")
    parser.add_argument("--groups-per-socket", type=int, default=4)
    parser.add_argument("--others-socket-index", type=int, default=2)
...
def main() -> int:
    ...
    report = build_report(
        ...
        report_title="NUMA Binding Report",
    )
