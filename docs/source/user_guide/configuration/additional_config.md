...
| `enable_cpu_binding`                | bool | `True`  | Whether to enable CPU binding. Only takes effect on ARM CPUs; when enabled, A3 uses NUMA-balanced binding and other device types use NUMA-affinity binding. Allowed CPUs are sharded by node-local rank to avoid overlap; if node detection fails, it falls back to global rank with `world_size_across_dp` (or `world_size` if unavailable). |
